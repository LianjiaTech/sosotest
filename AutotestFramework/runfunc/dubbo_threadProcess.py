from runfunc.dubbo_initial import *

# lock = threading.RLock() #线程锁

@catch_exception
@take_time
def thread_process_debug_queue():
    #DONE 处理调试队列 debugQueue  从这里实现多线程的分发，哪些user已经调试中需要停止线程等等。
    while True:
        if len(debugQueue) > 0 :
            try:
                logging.info("thread_process_debug_queue: debugQueue:%s" % debugQueue)
                debugType = debugQueue[0][Do.KEY_DO]
                logging.info("thread_process_debug_queue: 开始处理调试：%s" % debugQueue[0])

                if debugType == Do.TYPE_DEBUG_INTERFACE:
                    #进行接口调试
                    interfaceDebugId = int(debugQueue[0][Do.KEY_INTERFACE_DEBUG_ID])
                    dubboInterface = DubboInterface(interfaceDebugId = interfaceDebugId)
                    dubboInterface.generateByInterfaceDebugId()

                    if dubboInterface.addBy == None or dubboInterface.addBy.strip() == "":
                        logging.info("没有查到接口调试信息interfaceDebugId[%s]" % interfaceDebugId)
                        debugQueue.remove(debugQueue[0])
                        continue

                    addBy = dubboInterface.addBy
                    if dubboInterface.execStatus == ExecStatus.DONE or dubboInterface.execStatus == ExecStatus.EXCEPTION:
                        debugQueue.remove(debugQueue[0])
                    ...
                    tmpThread = DubboCaseDebugThread(debugType=debugType,dubboInterface = dubboInterface)
                    tmpThread.start()
                    debugThreadDict[addBy] = tmpThread
                elif debugType == Do.TYPE_DEBUG_CASE:
                    #进行用例调试
                    caseDebugId = debugQueue[0][Do.KEY_CASE_DEBUG_ID]
                    caseStepDebugIdList = debugQueue[0][Do.KEY_CASE_STEP_DEBUG_ID_LIST]
                    dubboCase = DubboTestcase()
                    dubboCase.generateByCaseDebugIdAndCaseStepDebugIdList(caseDebugId,caseStepDebugIdList)

                    if dubboCase.addBy == None or dubboCase.addBy.strip() == "":
                        logging.info("没有查到用例调试信息caseDebugId[%s]" % caseDebugId)
                        debugQueue.remove(debugQueue[0])
                        continue

                    addBy = dubboCase.addBy
                    if addBy in debugThreadDict.keys() and debugThreadDict[addBy].is_alive() :
                        stop_thread(debugThreadDict[addBy])

                    tmpThread = DubboCaseDebugThread(debugType=debugType, dubboCase = dubboCase)
                    tmpThread.start()
                    debugThreadDict[addBy] = tmpThread
                else:
                    #错误的调试类型
...
@catch_exception
@take_time
def thread_process_task_queue_SingleEnvToMultiTask():
    #TODO 单环境可同时执行多任务
    while True:
        taskStartIndex = 0
        while len(taskQueue) > taskStartIndex :
            logging.info("thread_process_task_queue: Processed_taskQueue:%s" % taskQueue)
            logging.info("thread_process_task_queue: 开始处理任务：%s" % taskQueue[taskStartIndex])
            ...
            try:
                ...
                if taskId not in serviceThreadDict[serviceConfKey].keys() or (serviceThreadDict[serviceConfKey][taskId]['thread'].isAlive() == False):
                    ...
                    tmpThread = DubbpTaskThread(task=taskQueue[taskStartIndex][Do.KEY_TASK_OBJECT])
                    tmpThread.start()
                    # if lock.acquire():
                    taskQueue.remove(taskQueue[taskStartIndex])
                    # lock.release()
                    ...
                    logging.info("thread_process_task_queue: 任务处理完毕")
                    break
                else:
                    #有进程且活着不执行当前任务,执行下一个
                    taskStartIndex += 1
