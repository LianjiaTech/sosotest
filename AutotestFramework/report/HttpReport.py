...
class HttpReport(BaseReport):
    ...
    def __init__(self):
        super(HttpReport, self).__init__()
...
<h1>HTTP测试报告</h1>""" % (self.report_html_head_content))
...
function toggleRetryCaseTrs(baseLineId){
    $("tr[nameRetry='retryCaseTr_"+baseLineId+"']").toggle();
        ...
        ##############任务摘要统计开始###############################################
        htmlFile.write("""<div id="summary_info" style="%s;display:block;">"""  % ((task.testResult == ResultConst.PASS and task.performanceResult == PerformanceConst.PASS) and self.success_color or self.danger_color))
...
<tr style="%s"> 
 ...
 <th width="10%%">性能通过</th>
 <th width="10%%">性能失败</th>
 <th width="10%%">性能N/A</th>
 <th width="10%%">性能通过率</th>
</tr>
</thead>
  <tbody>""" % self.warning_color)
        if task.actualTotalInterfaces != 0:
            htmlFile.write("""  <tr style="%s" id="interface_summary">""" % ((task.actualTotalInterfaces == task.interfacePassCount and task.interfacePerformancePassCount == task.actualTotalInterfaces) and self.success_color or self.danger_color))
...
<td><bdi>%d</bdi></td>
<td><bdi>%d</bdi></td>
<td><bdi>%.2f%%</bdi></td>""" % (task.actualTotalInterfaces,
                               task.interfacePassCount,
                               task.interfaceFailCount,
                                 (float(task.interfacePassCount) / float(task.actualTotalInterfaces) * 100),
                                task.interfacePerformancePassCount,task.interfacePerformanceFailCount,task.interfacePerformanceNotStatisticsCount,
                                 (float(task.interfacePerformancePassCount) / float(task.actualTotalInterfaces) * 100)))
            htmlFile.write("""</tr>""")

        if task.actualTotalTestcases != 0:
            htmlFile.write("""<tr style="%s" id="testcase_summary">""" % ((task.actualTotalTestcases == task.testcasePassCount and task.actualTotalTestcases == task.testcasePerformancePassCount) and self.success_color or self.danger_color))
...
<td><bdi>%.2f%%</bdi></td>""" % (
                               ...
                               task.testcasePerformancePassCount, task.testcasePerformanceFailCount,task.testcasePerformanceNotStatisticsCount,
                               (float(task.testcasePerformancePassCount) / float(task.actualTotalTestcases) * 100)))
            htmlFile.write("""</tr>""")

        if task.actualTotalInterfaces != 0 and task.actualTotalTestcases != 0:
            htmlFile.write("""<tr style="%s" id = "total_summay" >""" % ((task.actualTotal == task.passCount and task.actualTotal == task.performancePassCount) and self.success_color or self.danger_color))
...
<td><bdi>%.2f%%</bdi></td>""" % (
                               ...
                               task.performancePassCount,
                               task.performanceFailCount,
                               task.performanceNotStatisticsCount,
                           (float(task.performancePassCount) / float(task.actualTotal) * 100)))
...
<tr style="%s">
 ...
 <th width="5%%">性能N/A</th>
 <th width="6%%">性能通过率</th>
 <th width="6%%">性能记录数量</th>
 <th width="6%%">性能平均时间</th>
</tr>
</thead><tbody>""" % self.warning_color)
            for tmpKey,tmpValue in task.interfaceCountDict.items():
                if tmpValue['total'] != 0:
                    htmlFile.write("""<tr style="%s" >""" % ((tmpValue['total'] == tmpValue['pass'] and tmpValue['total'] == tmpValue['performancePass']) and self.success_color or self.danger_color))
...
""" % (tmpKey,
                       ...
                       (float(tmpValue['pass']) / float(tmpValue['total']) * 100) ,
                                   tmpValue['performancePass'],
                                   tmpValue['performanceFail'],
                                   tmpValue['performanceNA'],
                      (float(tmpValue['performancePass']) / float(tmpValue['total']) * 100),
                                   tmpValue['performanceCount'],
                                   (float(tmpValue['performanceCount'] != 0 and (float(tmpValue['performanceAllTime']) / float(tmpValue['performanceCount'])) or 0)),
...
<td>%s</td>
 ...
 """ % (tmpKey,tmpValue['host'],
                       ...
                       (float(tmpValue['performancePass']) / float(tmpValue['total'])) * 100,
                        tmpValue['performanceCount'],
                        (float(tmpValue['performanceCount'] != 0 and (
                        float(tmpValue['performanceAllTime']) / float(tmpValue['performanceCount'])) or 0)),
            ...
            for tmpKey, tmpValue in totalDict.items():
                if tmpValue['total'] != 0:
                    htmlFile.write("""<tr style="%s">""" % (
                        (tmpValue['total'] == tmpValue['pass'] and tmpValue['total'] == tmpValue["performancePass"]) and self.success_color or self.danger_color))
...
<label class="btn btn-primary">
   <input type="radio" name="options" id="optionPerformancePass" value="select" onChange="changeCaseStatus()" > 性能PASS
</label>
<label class="btn btn-primary">
   <input type="radio" name="options" id="optionPerformanceFail" value="select" onChange="changeCaseStatus()" > 性能FAIL
</label>
<label class="btn btn-primary">
...
<tbody>""")
            ...
            for i in range(0,len(interfaceList)):
                ...
                requestStr,responseStr = getHttpRequestAndResponseStr(interface)

                #获取className
                if interface.testResult == ResultConst.PASS and interface.interfacePerformanceResult == PerformanceConst.PASS:
                    className = "result_pass"
                if interface.testResult == ResultConst.PASS:
                    name1 = "result_pass"
                if interface.interfacePerformanceResult == PerformanceConst.PASS:
                    name2 = "result_performancePass"
                if interface.interfacePerformanceResult == PerformanceConst.FAIL:
                    className = "result_warning"
                    name2 = "result_performanceFail"
                ...
                if interface.testResult == ResultConst.NOTRUN:
                    ...
                    name1 = "result_notrun"
                if interface.interfacePerformanceResult == PerformanceConst.NA:
                    name2 = "result_performanceNA"
                ...
                htmlFile.write(""" <tr class="%s" name="resultTr" name1="%s" name2="%s" onclick="toggleTrBgClass($(this),'%s')">
<td>%d</td><td>%s</td>
...
<td>%s</td>
<td>
<a href="javascript:void(0)" onclick="ShowDiv('listshow_interface_%d','fade')">查看详情</a>
%s
...
<caption>%s[%s]&nbsp;&nbsp;请求：[%s]&nbsp;%s%s</caption>
   ...
   <tr class="result_pop"><td>HTTP请求</td><td>%s</td></tr>
   <tr class="result_pop"><td>HTTP响应</td><td>%s</td></tr>
   ...
   <td>执行前耗时：%s ms &nbsp;&nbsp;&nbsp;&nbsp;
       ...
       <br>总耗时：%s ms &nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
                ...
                html.escape(interface.url),
                html.escape(interface.testResult),
                html.escape(interface.interfacePerformanceResult),
                ...
                if len(interface.retryList) > 0:
                    ...
                    for retryIndex in range(0,len(interface.retryList)):
                        ...
                        # interface = HttpInterface() #TODO 编代码时提醒用，结束后注释掉。
                        requestStr, responseStr = getHttpRequestAndResponseStr(interfaceRetry)

                        # 获取className
                        if interfaceRetry.testResult == ResultConst.PASS and interfaceRetry.interfacePerformanceResult == PerformanceConst.PASS:
                            className = "result_pass"
                        if interfaceRetry.testResult == ResultConst.PASS:
                            name1 = "result_pass"
                        if interfaceRetry.interfacePerformanceResult == PerformanceConst.PASS:
                            name2 = "result_performancePass"
                        if interfaceRetry.interfacePerformanceResult == PerformanceConst.FAIL:
                            className = "result_warning"
                            name2 = "result_performanceFail"
                        ...
                        if interfaceRetry.testResult == ResultConst.NOTRUN:
                            className = "result_notrun"
                            name1 = "result_notrun"
                        if interfaceRetry.interfacePerformanceResult == PerformanceConst.NA:
                            name2 = "result_performanceNA"
                        htmlFile.write(""" <tr class="%s" style="display:none" name="retryTr_%d" name1="%s_retry" name2="%s_retry" nameRetryTr="1" onclick="toggleTrBgClass($(this),'%s')">
                        ...
                        <caption>%s[%s]&nbsp;&nbsp;请求：[%s]&nbsp;%s%s</caption>
                           ...
                           <td>执行前耗时：%s ms &nbsp;&nbsp;&nbsp;&nbsp;
                            ...
                            #html.escape(interfaceRetry.desc).replace("\n", "<br>"),
                            html.escape(interfaceRetry.url),
                            html.escape(interfaceRetry.testResult),
                            html.escape(interfaceRetry.interfacePerformanceResult),
                            interfaceRetry.performanceTime,
                            ...
                            html.escape(interfaceRetry.interfaceId),
                            html.escape(interfaceRetry.title),
                            interfaceRetry.method,
                            html.escape(interfaceRetry.host),
                            html.escape(interfaceRetry.url),
...
<tbody>""")
            ...
            for i in range(0,len(testcaseList)):
                ...
                #获取className
                if httpTestcase.testResult == ResultConst.PASS and httpTestcase.testcasePerformanceResult == PerformanceConst.PASS:
                    className = "result_pass"
                if httpTestcase.testResult == ResultConst.PASS:
                    name1 = "result_pass"
                if httpTestcase.testcasePerformanceResult == PerformanceConst.PASS:
                    name2 = "result_performancePass"
                if httpTestcase.testcasePerformanceResult == PerformanceConst.FAIL:
                    className = "result_warning"
                    name2 = "result_performanceFail"
                ...
                if httpTestcase.testResult == ResultConst.NOTRUN:
                    className = "result_notrun"
                    name1 = "result_notrun"
                if httpTestcase.testcasePerformanceResult == PerformanceConst.NA:
                    name2 = "result_performanceNA"
...
<td>""" % (#第一行
                ...
                httpTestcase.testcasePerformanceResult,
...
<tr class="stepDivs" id="steplistshow_%d" style="display:none;"><td colspan="14">
<div>
...
<th width="3%%">步骤</th>
<th width="20%%">描述</th>
<th width="15%%">请求主机URI</th>
<th width="20%%">请求地址URL</th>
<th width="5%%">测试结果</th>
<th width="5%%">性能结果</th>
<th width="5%%">性能预期</th>
<th width="5%%">执行前耗时</th>
...
<tbody>""" % (i,toggleRetryTrsHtml,i))
                ...
                for stepi in range(0,len(httpTestcase.stepTestcaseList)):
                    ...
                    requestStr, responseStr = getHttpRequestAndResponseStr(tmpStep)
...
<br>总耗时：%s ms &nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
                      ...
                      tmpStep.url,
                      tmpStep.testResult,
                      tmpStep.testcaseStepPerformanceResult,
                      tmpStep.performanceTime,
                ...
                if len(httpTestcase.retryList) > 0:
                    ...
                    for retryIndex in range(0, len(httpTestcase.retryList)):
                        ...
                        # httpTestcase = HttpTestcase()
                        # 获取className
                        if httpTestcaseRetry.testResult == ResultConst.PASS and httpTestcaseRetry.testcasePerformanceResult == PerformanceConst.PASS:
                            className = "result_pass"
                        if httpTestcaseRetry.testResult == ResultConst.PASS:
                            name1 = "result_pass"
                        if httpTestcaseRetry.testcasePerformanceResult == PerformanceConst.PASS:
                            name2 = "result_performancePass"
                        if httpTestcaseRetry.testcasePerformanceResult == PerformanceConst.FAIL:
                            className = "result_warning"
                            name2 = "result_performanceFail"
                        ...
                        if httpTestcaseRetry.testResult == ResultConst.NOTRUN:
                            className = "result_notrun"
                            name1 = "result_notrun"
                        if httpTestcaseRetry.testcasePerformanceResult == PerformanceConst.NA:
                            name2 = "result_performanceNA"


                        htmlFile.write("""<tr class="%s" style="display:none" nameRetry="retryCaseTr_%d" name="resultTr" name1="%s_retry" name2="%s_retry"  nameRetryTr="1" onclick="toggleTrBgClass($(this),'%s')">
                        ...
                        <td>""" % (  # 第一行
                            ...
                            httpTestcaseRetry.testcasePerformanceResult,
                        ...
                        <tr class="stepDivs" id="steplistshow_%d_%d" style="display:none;"><td colspan="14">
                        <div>
                        <table class="table table-bordered" style="word-break:break-all; word-wrap:break-all;">
                        <thead>
                        <tr>
