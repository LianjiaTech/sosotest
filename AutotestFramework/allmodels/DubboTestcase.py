from allmodels.DubboTestcaseStep import DubboTestcaseStep
...
class DubboTestcase(CommonAttr):
    ...
    def execute(self):
        ...
        Returns:
            无
        """
        logging.debug("****************************************DUBBO用例TESTCASE[%s]开始执行在环境[%s]执行人[%s]***********************************************" % (self.caseId,self.confHttpLayer.key,self.addBy))
        ...
        for tmpStepIndex in range(0,len(self.stepTestcaseList)):
            ...
            if tmpStepResult.testResult == ResultConst.PASS:
                ...
                break
        logging.debug("****************************************用例[%s]结束执行***********************************************" % self.caseId)
    ...
    def generateByCaseDebugIdAndCaseStepDebugIdList(self,caseDebugId, caseStepDebugIdList=""):
        ...
        Args:
            caseDebugId: tb2_dubbo_testcase_debug中的id
            caseStepDebugIdList:tb_http_testcase_step_debug中的id LIST

        Returns:
            无
        """
        # sqlCaseDebug = "select * from tb2_dubbo_testcase_debug where id = %d" % int(caseDebugId)
        ...
        #         httpCaseStep = DubboTestcaseStep()
        #         httpCaseStep.id = tmpStepDebugId
        #         httpCaseStep.caseStepDebugId = tmpStepDebugId
        #         httpCaseStep.generateByCaseStepDebugId()
        #         if httpCaseStep.stepSwitch == TestCaseStepSwitch.USE_SWITCH:
        ...
        for tmpStepDebugId in caseStepDebugIdList:
            if int(tmpStepDebugId["stepSwitch"]) == TestCaseStepSwitch.USE_SWITCH:
                caseStep = DubboTestcaseStep()
                # httpCaseStep.id = tmpStepDebugId
                caseStep.caseStepDebugData = tmpStepDebugId
                caseStep.generateByStepDebugDict(tmpStepDebugId)

                caseStep.confHttpLayer = self.confHttpLayer
                self.addStep(caseStep)




    @catch_exception
    def addStep(self,stepCase = DubboTestcaseStep()):
        """
        添加步骤
        Args:
            stepCase: 要添加的step对象
        ...
        Returns:
            无
        """
        if type(stepCase) == type(DubboTestcaseStep()):
            self.stepTestcaseList.append(stepCase)

    # @catch_exception
    # def updateByCaseDebugId(self):
    #     """
    #     根据id更新tb2_dubbo_testcase_debug表的测试结果，包括步骤结果
    #     Returns:
    #         无。
    #     """
    #     #更新各个步骤的结果
    ...
    #     sql = """ UPDATE tb2_dubbo_testcase_debug SET testResult='%s',assertResult='%s',
    ...
    def generateByCaseId(self):
        ...
        sqlGetCase = "select * from tb_http_testcase where caseId='%s'" % tmpCaseId
        ...
        if resCaseResult:
            ...
            for tmpCaseStepDict in resCaseStep:
                if tmpCaseStepDict["stepSwitch"] == TestCaseStepSwitch.USE_SWITCH:
                    tmpHttpCaseStep = DubboTestcaseStep()
