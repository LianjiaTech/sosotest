...
from apps.dubbo_task.services.dubbo_taskService import *
from apps.dubbo_task_suite.services.dubbo_taskSuiteService import *
from apps.common.decorator.permission_normal_funcitons import *
from apps.dubbo_interface.services.dubbo_interface_service import *
from apps.common.func.WebFunc import getServiceConf


def dubboTaskSuiteCheck(request):
    langDict = getLangTextDict(request)

    context = {}
    if not isRelease:
        context["env"] = "test"
    context["dubbo_taskSuiteCheck"] = "current-page"
    ...
    text["pageTitle"] = "DUBBO任务集查看"
    context["text"] = text

    context["page"] = 1

    # context["lang"] = getLangTextDict(request)
    return render(request, "dubbo/taskSuite/taskSuiteCheck.html", context)
...
def dubbo_taskSuiteListCheck(request):
    ...
    return render(request, "dubbo/taskSuite/SubPages/task_suite_list_check_page.html",context)
    # return response
...
def getTaskSuiteForTaskSuiteId(request):
    ...
    return render(request, "dubbo/taskSuite/SubPages/task_suite_run_details_page.html", context)
...
def dubboTaskSuiteSelectTaskList(request):
    ...
    execSql = "SELECT i.*,u.userName from %s i LEFT JOIN tb_user u ON i.addBy = u.loginName  WHERE 1=1 and i.state=1 %s " % (tbName,versionCondition)
    checkList = []
    for key in checkArr:
        if checkArr[key] == "":
            continue
    ...
    response = render(request,"dubbo/taskSuite/SubPages/task_suite_details_select_task_list_check_page.html", context)
    return response

@single_data_permission(Tb2DUBBOTaskSuite,Tb2DUBBOTaskSuite)
def taskSuiteAddData(request):
    taskSuiteData = json.loads(request.body)

    taskSuiteData["protocol"] = "DUBBO"

    if VersionService.isCurrentVersion(request):
        taskSuiteData["addBy"] = request.session.get("loginName")

        createTask = dubbo_taskSuiteService.addTaskSuite(taskSuiteData)
        if createTask.id >= 1:
...
def taskSuitSaveEdit(request):
    ...
    if VersionService.isCurrentVersion(request):
        ...
        editTaskData = dubbo_taskSuiteService.editTaskSuite(taskSuiteData)
        if editTaskData == 1:
            return HttpResponse(ApiReturn(ApiReturn.CODE_OK).toJson())
        else:
...
def operationTaskSuite(request,context):
    ...
    try:
        if VersionService.isCurrentVersion(request):
            context["dataAddBy"] = dubbo_taskSuiteService.getTaskSuiteForId(request.GET.get("id")).addBy
        else:
            context["dataAddBy"] = dubbo_taskSuiteService.getTaskSuiteForId(request.GET.get("id")).addBy
...
def getTaskSuiteData(request):
    ...
    if VersionService.isCurrentVersion(request):
        ...
        for i in range(0,len(taskSuiteTaskList)):
            try:
                thisTask = DubboTaskService.getTaskForTaskId(taskSuiteTaskList[i])
                if not thisTask:
                    continue
...
def getTaskListDataForTaskSuite(request):
    ...
    if VersionService.isCurrentVersion(request):
        ...
        getTaskDataSql = taskData["taskList"].replace(",","' union all select thi.id,thi.taskId,thi.title,thi.taskdesc,thi.interfaceNum,thi.addBy,tu.userName from tb2_dubbo_task thi  LEFT JOIN tb_user tu on thi.addBy = tu .loginName  where taskId = '")
    ...
    response = render(request,"dubbo/taskSuite/SubPages/task_list_check_page.html", {"pageDatas":taskInterfaceListData})
    return response


def taskSuiteRunAdd(request):
    if VersionService.isCurrentVersion(request):
        taskSuiteData = dubbo_taskSuiteService.getTaskSuiteForTaskSuiteId(request.POST.get("taskSuiteId"))
    else:
        taskSuiteData = dubbo_taskSuiteService.getTaskSuiteForTaskSuiteId(request.POST.get("taskSuiteId"))
    ...
    for taskIndex in taskList:
        ...
        for httpConfIndex in range(0,len(httpConfList)):
            ...
            RedisCache().set_data("%s_taskSuite_%s_task_%s" % ("DUBBO",taskSuiteExecuteModel.id,taskExecuteData.id), json.dumps({"progress":0,"testResult":"","execStatus":0}), 60 * 60 * 12)
    ...
    taskSuiteRedisDict = {"taskExecuteIdList": taskExecuteIdList, "execStatus": 1, "progress": 0}
    ...
    for index in taskExecuteTcpList:
        ...
        if retApiResult.code != ApiReturn.CODE_OK:
            ...
            for taskIndex in taskExecuteTcpList:
                RedisCache().del_data("taskExecute_%s" % json.loads(taskIndex)['TaskExecuteId'])
...
def taskSuiteResultCheck(request):
    ...
    context["page"] = 1
    return render(request, "dubbo/taskSuite/task_suite_ExecResult.html", context)
...
def getTaskSuiteResultList(request):
    ...
    if isSqlInjectable(orderBy):
        return HttpResponse("<script>alert('查询条件非法');</script>")
    execSql = "SELECT t.*,u.userName from tb2_dubbo_task_suite_execute t LEFT JOIN tb_user u ON t.addBy = u.loginName  WHERE  (t.execStatus between 1 and 2) or (t.state=1"
    checkList = []
        ...
        try:
            if pageData["execStatus"] == 2:
                ...
                for taskIndex in taskExecuteIdList:
                    taskExecStatus = json.loads(redisCache.get_data("%s_taskSuite_%s_task_%s" % ("DUBBO",pageData["id"], taskIndex)))
    ...
    response = render(request, "dubbo/taskSuite/SubPages/task_suite_result_list_page.html", context)
...
def updateTaskSuiteExecuteProgressData(request):
    ...
    try:
        for idIndex in taskExecuteIdList:
            ...
            taskSuiteExecuteData = json.loads(redisCache.get_data("%s_taskSuiteExecuteId_%s" % ("DUBBO",idIndex)))
            progressList = []
            for taskIndex in taskSuiteExecuteData["taskExecuteIdList"]:
                taskExecStatus = json.loads(redisCache.get_data("%s_taskSuite_%s_task_%s" % ("DUBBO",idIndex, taskIndex)))
                # print("""taskExecStatus["execStatus"] """,taskExecStatus["execStatus"] )
...
def againRunTaskSuite(request):
    if VersionService.isCurrentVersion(request):
        ...
        taskSuiteData = Tb2DUBBOTaskSuiteExecute.objects.get(id = request.GET.get("taskSuiteId"))
    else:
        taskSuiteData = Tb2DUBBOTaskSuiteExecute.objects.get(id = request.GET.get("taskSuiteId"))
    ...
    taskSuiteExecuteModel.save(force_update=True)
    taskSuiteRedisDict = {"taskExecuteIdList": taskExecuteIdList, "execStatus": 1, "progress": 0}
    RedisCache().set_data("%s_taskSuiteExecuteId_%s" % ("DUBBO",taskSuiteExecuteModel.id), json.dumps(taskSuiteRedisDict),
                          60 * 60 * 12)
    for index in taskExecuteTcpList:
        retApiResult = send_tcp_request(index)
        if retApiResult.code != ApiReturn.CODE_OK:
...
def stopTaskSuiteRun(request):
    ...
    try:
        taskSuite = Tb2DUBBOTaskSuiteExecute.objects.get(id=taskSuiteExecuteId)
...
def getSelectExecuteStatus(request):
    sql = "SELECT testResult,count(*) as count from tb2_dubbo_task_suite_execute GROUP BY testResult"
