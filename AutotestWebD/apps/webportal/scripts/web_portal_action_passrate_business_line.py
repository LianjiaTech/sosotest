    ...
    for num in numList:
        ...
        for businessLine in businessLinesList:
            ...
            for httpConf in httpConfKeysList:
                ...
                taskList = TbWebPortalStandardTask.objects.filter(version=version, businessLine=businessLine, state=1)
                passRate = 0
                for task in taskList:
                    ...
                    for taskExecute in taskExecuteResult:
                        ...
                        if not isJson(testResultMsg) or "totalExecuteSummary" not in json.loads(
                                testResultMsg).keys():
                            continue
                ...
                envPassRateList.append({"env": httpConf, "passRate": passRate, "date": date.strftime("%Y-%m-%d"),
                                        "alias": alias + "(" + version + ")", "passNum": passNum,
                                        "totalNum": totalNum})
