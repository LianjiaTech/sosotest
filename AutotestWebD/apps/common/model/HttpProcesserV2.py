...
from apps.common.model.TypeTool import TypeTool
# 禁用安全请求警告
...
class HttpProcesserV2(object):
    ...
    """
    def __init__(self,method = "POST",host = "",url = "/",headers = None,params = None,bodyType = None,bodyContent=None,sess = requests.session(),timeout = 20):
        self.supportMethodList = ["get", "post", "options", "head", "put", "patch", "delete"]
        self.bodyTypeList = ["form-data","x-www-form-urlencoded","raw","binary"]
    ...
    def sendRequest(self):
        ...
        if self.method in self.supportMethodList :
            ...
            try:
                ...
                if self.bodyType == self.bodyTypeList[0]:
                    #form-data
                    return eval("self.sess.%s(self.host + self.url+paramStr, headers=self.headers,  files=self.bodyContent,timeout=self.timeout, verify=False)" % self.method)
                elif self.bodyType in self.bodyTypeList or self.bodyType == None:
                    # x-www-form-urlencode
                    # raw json text xml 等
                    # binary
                    return eval("self.sess.%s(self.host + self.url+paramStr, headers=self.headers,  data=self.bodyContent,timeout=self.timeout, verify=False)" % self.method)
                else:
